...
def test_codec():
    ...
    train_data = widerface.crop(train_data, 10000, image_size)
    for sample in train_data:
        image = imgkit.crop(Image.open(sample["image"]), sample["crop"])
        boxes = np.array(sample["boxes"])
        feature = encode(image.size, boxes, (4,3,5))
        print(feature)
        boxes=decode(image.size, feature, 1.0)
        imgkit.draw_grids(image, (4,3))
...
def train_model(model, train_data, image_size, feature_shape, batch_size):
    ...
    for i in range(111):
        ...
        model.save(model_file + "_" + str(380+(i+1)*20) + ".h5")
...
def test_model():
    ...
    # build model
    model_file = os.path.dirname(os.path.abspath(__file__)) + "/datasets/widerface/face_model_v4_380.h5"
